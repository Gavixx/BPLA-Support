...
#include <memory> // Додано для std::unique_ptr
...
void __fastcall TForm6::ButtonAddUserClick(TObject *Sender)
{
    ...
    if (EditName->Text.Trim().IsEmpty() || EditMail->Text.Trim().IsEmpty() ||
        EditNumber->Text.Trim().IsEmpty() || ComboBoxRole->Text.Trim().IsEmpty() ||
        ComboBoxOrg->Text.Trim().IsEmpty())
    {
        ShowMessage("Будь ласка, заповніть всі поля.");
        return;
    }
    if (EditPassword->Text != EditConfiirmPassword->Text)
    {
    ...
    try
    {
        ...
        // Конвертація UnicodeString до std::string (UTF-8)
        std::string password = std::string(UTF8String(EditPassword->Text).c_str());
        ...
        // Конвертація std::string (hashedPassword) до UnicodeString
        UnicodeString hashedPasswordStr = String(hashedPassword.c_str());

        insertCredentialsQuery->ParamByName("password")->AsString = hashedPasswordStr;
