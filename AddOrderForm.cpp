...
#include "DataModule.h"
	...
	void __fastcall TForm5::ButtonAddClick(TObject *Sender)
	{
		...
		FDQuery1->ParamByName("drone_name")->AsString = LabeledEditDroneName->Text;  // Передаємо параметр drone_name
		...
		if (!FDQuery1->Eof) {
			...
			// Якщо запису немає, додаємо новий дрон
			FDQuery1->Close();  // Закрити запит
			...
			FDQuery1->ExecSQL();  // Виконати вставку

			// Після вставки отримуємо ID новоствореного дрона
			FDQuery1->Close();  // Закрити запит
			FDQuery1->SQL->Text = "SELECT LAST_INSERT_ID() AS drone_id";  // Отримуємо останній вставлений ID
			FDQuery1->Open();  // Виконати запит
			drone_id = FDQuery1->FieldByName("drone_id")->AsInteger;
		}

		FDQuery1->Close();  // Закрити запит після отримання drone_id

		// Вставляємо новий запит у MilitaryRequests
